...
from StableDiffusion import create_and_save_figure
...
for index in range(len(character_list)):
    if create_new_character:
        create_and_save_figure(character_list[index]["description"], f"{cwd}{index+1}.png")
    character_list[index]["image"] = f"{cwd}{index+1}.png"
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if current_match < len(match_pairs) and next_button_rect.collidepoint(event.pos):
                    ...
                    if any(character["score"] >= 5 for character in characters):
